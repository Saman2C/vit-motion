...
2. ``integrated_gradients`` -> image vs. each (step, channel) numeric input, as a
                             joint attribution over the image and the [B,K,5]
                             sequence (Integrated Gradients, Sundararajan 2017),
                             including a modality share (image vs. numeric).
...
class MotionInterpreter:
    ...
    def integrated_gradients(self, image, numeric, image_age=None, target="yaw", steps=32):
        ...
        denom = img_total + num_total + 1e-12
        modality = {
            "image": img_total / denom,
            "numeric": num_total / denom,
        }
...
def render_explanation(exp: Explanation, save_path=None, title=None):
    ...
    if exp.ig_numeric is not None:
        ...
        ig_share = exp.modality_share.get("integrated_gradients", {})
        ax.set_title(
            "Integrated Gradients (numeric)\n"
            f"image {ig_share.get('image', 0):.2f}  vs  numeric {ig_share.get('numeric', 0):.2f}"
        )
